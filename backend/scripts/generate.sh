...
configtxgen -profile LRANOrdererGenesis -outputBlock ./channel-artifacts/genesis.block
configtxgen -profile LRANChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID mainchannel
...
OPTS="-i"
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/citizen.lran.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/lawyer.lran.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/registryoffice.lran.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/blro.lran.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
